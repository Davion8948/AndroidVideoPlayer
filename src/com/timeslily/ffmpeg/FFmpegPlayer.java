...
public class FFmpegPlayer {
    ...
    private static class SetDataSourceTask extends AsyncTask<Object, Void, SetDataSourceTaskResult> {
        ...
        protected void onPostExecute(SetDataSourceTaskResult result) {
            if (player.mpegListener != null)
                player.mpegListener.onFFDataSourceLoaded(result.error, result.streams);
        }
    ...
    private Runnable updateTimeRunnable = new Runnable() {
        ...
        public void run() {
            if (mpegListener != null) {
                mpegListener.onFFUpdateTime(mCurrentTimeUs, mVideoDurationUs, mIsFinished);
            }
        }
    ...
    private native int setDataSourceNative(String url, Map<String, String> dictionary, int videoStreamNo, int audioStreamNo, int subtitleStreamNo);
    ...
    public void seek(long positionUs) {//seek
    ...
    private Bitmap prepareFrame(int width, int height) {
        Log.i("ccc", "prepareFrame.......width=" + width + "   height=" + height);
    ...
    private AudioTrack prepareAudioTrack(int sampleRateInHz, int numberOfChannels) {
        ...
        for (;;) {
            ...
            if (numberOfChannels == 1) {
                ...
                channelConfig = AudioFormat.CHANNEL_OUT_FRONT_CENTER | AudioFormat.CHANNEL_OUT_FRONT_RIGHT | AudioFormat.CHANNEL_OUT_FRONT_LEFT;
            ...
            try {
                ...
                AudioTrack audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRateInHz, channelConfig, AudioFormat.ENCODING_PCM_16BIT,
                ...
                return null;//若出现采样率过大的bug，返回空在VideoActivity中处理异常，无法播放此视频
            }
        }
    }
    ...
    public void setDataSource(String url, Map<String, String> dictionary, int videoStream, int audioStream, int subtitlesStream) {
        new SetDataSourceTask(this).execute(url, dictionary, Integer.valueOf(videoStream), Integer.valueOf(audioStream),
    ...
    public static native int getDuration(String filePath);
     ...
     * @param height
     */
    public void setOnVideoSize(final int width, final int height) {
        activity.runOnUiThread(new Runnable() {//必须在主线程中更新
