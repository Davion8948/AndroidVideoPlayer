...
void player_print_codec_description(const struct AVCodec *codec){
	...
	//LOGI(3,"player_print_codec_description id: %d codec: %s, type: %s",codec->id, codec->name, type);
}
 ...
 struct Player{
	...
	jmethodID player_set_video_size;
	...
	DECLARE_ALIGNED(16,uint8_t,audio_buf2)[192000 * 4];
...
enum PlayerErrors {
	...
	ERROR_NOT_FOUND_SET_ON_VIDEO_SIZE,
...
struct Player * player_get_player_field(JNIEnv *env, jobject thiz){
	jfieldID m_native_layer_field = java_get_field(env, player_class_path_name, player_m_native_player);
	struct Player * player = (struct Player *)(*env)->GetIntField(env, thiz,m_native_layer_field);
	return player;
}
/*释放内存*/
...
void player_print_report_video_streams_free(JNIEnv *env, jobject thiz, struct Player *player){
	if(player->player_set_stream_info_method!=NULL){
		(*env)->CallVoidMethod(env, thiz, player->player_set_stream_info_method, NULL);
	}
}

/*seek*/
void jni_player_seek(JNIEnv *env, jobject thiz, jlong positionUs){
	LOGI(2, "jni_player_seek");
   struct Player *player = player_get_player_field(env, thiz);
   pthread_mutex_lock(&player->mutex_operation);

   if(!player->playing){//没播放
		LOGI(1, "jni_player_seek could not seek while not playing");
		throw_exception(env, not_playing_exception_class_path_name, "Could not pause while not playing");
		goto end;
   }
...
void jni_player_pause(JNIEnv *env, jobject thiz){
    ...
    LOGI(2, "jni_player_pause Pausing");
    player->pause = TRUE;
...
void jni_player_resume(JNIEnv *env, jobject thiz){
	LOGI(2, "jni_player_resume");
	struct Player *player  = player_get_player_field(env, thiz);
	pthread_mutex_lock(&player->mutex_operation);

	if(!player->playing){
		LOGI(1, "jni_player_resume could not resume while not playing");
...
void player_play_prepare_free(struct Player *player){
	pthread_mutex_lock(&player->mutex_queue);
	player->stop = TRUE;
	pthread_cond_broadcast(&player->cond_queue);// 该函数用来对所有等待参数cond所指定的条件变量的线程解除阻塞
	pthread_mutex_unlock(&player->mutex_queue);
}
void player_alloc_queues_free(struct State *state){
	...
	for(stream_no=0;stream_no<capture_streams_no;++stream_no){
		if(player->packets[stream_no]!=NULL){
			queue_free(player->packets[stream_no], &player->mutex_queue, &player->cond_queue, state);
...
void player_create_audio_track_free(struct Player *player, struct State *state){
    ...
    if(player->audio_track != NULL){
    	...
    	(*state->env)->CallVoidMethod(state->env, player->audio_track, player->audio_track_pause_method);
...
int player_open_input(struct Player *player, const char * file_path, AVDictionary *dictionary){
	int ret;
	if((ret=avformat_open_input(&(player->input_format_ctx), file_path, NULL, &dictionary))<0){
		char errbuf[128];
		const char *errbuf_ptr = errbuf;
		if(av_strerror(ret, errbuf, sizeof(errbuf))<0){
			errbuf_ptr = strerror(AVUNERROR(ret));
		}
		LOGE(1,"player_set_data_source Could not open video file: %s (%d: %s)\n",file_path, ret, errbuf_ptr);
...
int player_create_interrupt_callback(struct Player *player){
	...
	player->interrupt_callback = (AVIOInterruptCB){player_ctx_interrupt_callback, player};
...
void player_print_video_informations(struct Player * player, const char * file_path){
	if(LOG_LEVEL>3){
		...
		for(i=0;i<player->input_format_ctx->nb_streams;i++){
			...
			LOGI(3, "--metadata:");
			while((tag=av_dict_get(metadaat, "",tag, AV_DICT_IGNORE_SUFFIX))!=NULL){
...
int player_print_report_video_streams(JNIEnv *env, jobject thiz, struct Player *player){
	int i;
	int err = ERROR_NO_ERROR;
	jclass stream_info_class = (*env)->FindClass(env, stream_info_class_path_name);
	jmethodID stream_info_set_metadata_method = java_get_method(env, stream_info_class, stream_info_set_metadata);
	jmethodID stream_info_set_media_type_internal_method = java_get_method(env, stream_info_class, stream_info_set_media_type_internal);
	jmethodID stream_info_set_stream_number_method = java_get_method(env, stream_info_class, stream_info_set_stream_number);
	jmethodID stream_info_constructor = java_get_method(env, stream_info_class,empty_constructor);

	jclass hash_map_class = (*env)->FindClass(env, hash_map_class_path_name);
	jmethodID hash_map_constructor = java_get_method(env, hash_map_class, empty_constructor);

	jclass map_class = (*env)->FindClass(env, map_class_path_name);
	jmethodID map_put_method = java_get_method(env, map_class, map_put);

	jobjectArray array = (*env)->NewObjectArray(env, player->input_format_ctx->nb_streams, stream_info_class, NULL);
	if(array==NULL){
		err = -ERROR_COULD_NOT_ALLOCATE_MEMORY;
		goto free_map_class;
	}
	for(i=0;i<player->input_format_ctx->nb_streams&&err==ERROR_NO_ERROR;i++){
		AVStream *stream = player->input_format_ctx->streams[i];
		AVCodecContext *codec = stream->codec;
		AVDictionary *metadaat = stream->metadata;
		AVDictionaryEntry *tag = NULL;

		jobject stream_info = (*env)->NewObject(env, stream_info_class, stream_info_constructor);
		if(stream_info ==NULL){
			err = -ERROR_COULD_NOT_ALLOCATE_MEMORY;
			break;
		}
        jobject map = (*env)->NewObject(env, hash_map_class, hash_map_constructor);
        if(map==NULL){
        	err = -ERROR_COULD_NOT_ALLOCATE_MEMORY;
        ...
        (*env)->CallVoidMethod(env, stream_info, stream_info_set_media_type_internal_method, (jint)codec_type);
        (*env)->CallVoidMethod(env, stream_info, stream_info_set_stream_number_method, (jint)i);

        while(err==ERROR_NO_ERROR&&(tag=av_dict_get(metadaat, "", tag, AV_DICT_IGNORE_SUFFIX))!=NULL){
        ...
        (*env)->CallVoidMethod(env, stream_info, stream_info_set_metadata_method, map);
	...
	if(err == ERROR_NO_ERROR){
		(*env)->CallVoidMethod(env, thiz, player->player_set_stream_info_method, array);
	}
...
int player_open_stream(struct Player *player, AVCodecContext *ctx, const struct AVCodec **codec , int stream_no){
	enum AVCodecID codec_id = ctx->codec_id;
	LOGI(3, "player_open_stream trying open: %d", codec_id);
	*codec = avcodec_find_decoder(codec_id);
	if(*codec==NULL){
		LOGE(1, "player_set_data_source Could not find codec for id: %d",codec_id);
...
int player_try_open_stream(struct Player *player, enum AVMediaType codec_type, int stream_no){
	if(stream_no<0){
		return -1;
	}
...
int player_find_stream(struct Player *player, enum AVMediaType codec_type, int recommended_stream_no){
	...
	int err = ERROR_NO_ERROR;
	LOGI(3, "pllayer_find_stream, type:%d", codec_type);
	int bn_stream = player_try_open_stream(player, codec_type, recommended_stream_no);
	if(bn_stream<0){
		int i;
		for(i=0;i<player->input_format_ctx->nb_streams;i++){
    ...
    if(codec_type==AVMEDIA_TYPE_VIDEO){
    	LOGI(2,"player_set_data_source Video size is [%d x %d]", player->input_codec_ctxs[streams_no]->width,
...
int player_alloc_video_frames(JNIEnv *env, struct Player *player){
	player->rgb_frame = avcodec_alloc_frame();//分配内存去贮存解码后的数据
	if(player->rgb_frame==NULL){
    ...
    player->tmp_buffer = (uint8_t *)av_malloc(numBytes*sizeof(uint8_t));//分配buffer
    if(player->tmp_buffer == NULL){
    ...
    avpicture_fill((AVPicture *)player->tmp_frame, player->tmp_buffer, PIX_FMT_RGBA, ctx->width, ctx->height);//将buffer与rgb_frame绑定 buffer中存放yuv数据， fill之后将yuv放到rgb_frame的uint8_t data[4];
    avpicture_fill((AVPicture *)player->tmp_frame2, player->tmp_buffer2, PIX_FMT_RGBA, ctx->width, ctx->height);
	LOGI(2, "Allocating: %dx%d", ctx->width, ctx->height);
	(*env)->CallVoidMethod(env, player->thiz, player->player_set_video_size, ctx->width, ctx->height);
	return 0;
}
...
int player_alloc_queues(struct State *state){
	struct Player *player = state->player;
	int capture_stream_no = player->caputre_streams_no;
	int stream_no;
	for(stream_no = 0; stream_no < capture_stream_no; ++stream_no){//从0到capture_stream_no-1的queue
		player->packets[stream_no] = queue_init_with_custom_lock(50,//队列
...
int player_create_audio_track(struct Player *player, struct State *state){
	...
	jobject audio_track = (*state->env)->CallObjectMethod(state->env, player->thiz, player->player_prepare_audio_track_method, sample_rate, channels);
    ...
    player->audio_track_channel_count = (*state->env)->CallIntMethod(state->env, player->audio_track, player->audio_track_get_channel_count_method);
    int audio_track_sample_rate = (*state->env)->CallIntMethod(state->env, player->audio_track, player->audio_track_get_sample_rate_method);
    ...
    int64_t dec_channel_layout = (ctx->channel_layout&&ctx->channels==av_get_channel_layout_nb_channels(ctx->channel_layout))?ctx->channel_layout:av_get_default_channel_layout(ctx->channels);
    player->swr_context = NULL;

    if(ctx->sample_fmt != player->audio_track_format||dec_channel_layout!=audio_track_layout||ctx->sample_rate!=audio_track_sample_rate){
    	LOGI(2,	"player_set_data_sourcd preparing conversion of %d Hz %s %d channels to %d Hz %s %d channels",
    		    ctx->sample_rate,
		...
		if (!player->swr_context || swr_init(player->swr_context) < 0) {
				LOGE(1,	"Cannot create sample rate converter for conversion "
...
void player_get_video_duration(struct Player *player){//为什么获取三遍
	...
	for(i=0; i<player->caputre_streams_no;++i){
		...
		if(stream->duration>0){
			player->video_duration = av_rescale_q(stream->duration, stream->time_base, AV_TIME_BASE_Q);
//			LOGI(3, "player_set_data_source stream[%d] duration: %ld", i, stream->duration);
			return ;
		}
	}
	if(player->input_format_ctx->duration != 0){
		player->video_duration = player->input_format_ctx->duration;
//		LOGI(3,	"player_set_data_source video duration: %ld",player->input_format_ctx->duration)
		return;
	}
	for(i=0;i<player->input_format_ctx->nb_streams;i++){
		AVStream *stream = player->input_format_ctx->streams[i];
...
void player_update_current_time(struct State *state, int64_t current_time, int64_t video_duration, int is_finished){
	...
	LOGI(4, "player_update_current_time: %f/%f",current_time/1000000.0, video_duration/1000000.0);
	(*state->env)->CallVoidMethod(state->env, player->thiz, player->player_on_update_time_method, current_time, video_duration, jis_finished);
}
void player_update_time(struct State *state, int is_finished){
	...
	if(time_diff>500000ll || time_diff<-500000ll){//5秒？
		player->last_updated_time = current_video_time;
		inform_user = TRUE;
	}
    int64_t video_duration = player->video_duration;
    pthread_mutex_unlock(&player->mutex_queue);
	LOGI(6, "player_update_time: %f/%f",current_video_time/1000000.0, video_duration/1000000.0);
	if(inform_user){
        ...
        player_update_current_time(state, current_video_time, video_duration, is_finished);
	}
}
...
//	LOGI(2, "player_wait_for_frame[%d] start", stream_no);
	pthread_mutex_lock(&player->mutex_queue);
	...
	while(1){
        ...
        if(player->pause){
        	LOGI(2,"pauseeeeeeeeeeeeeeeeeeeeeeee");
        	pthread_cond_wait(&player->cond_queue, &player->mutex_queue);//等待？
        	continue;
        }
        int64_t current_video_time = player_get_current_video_time(player);
    	LOGI(8, "player_wait_for_frame[%d = %s] = (%f) - (%f)",
    			stream_no,
    			player->video_stream_no == stream_no ? "Video" : "Audio",
    			stream_time/1000000.0,
    			current_video_time/1000000.0);
    	int64_t sleep_time = stream_time - current_video_time;
    	LOGI(8,	"player_wait_for_frame[%d] Waiting for frame: sleeping: %" SCNd64,
    				stream_no, sleep_time);
    	if(sleep_time < -300000ll){
    		//300ms lata
    		int64_t new_value = player->start_time - sleep_time;
    		LOGI(4,	"player_wait_for_frame[%d] correcting %f to %f because late",
        ...
        int timeout_ret = pthread_cond_timeout_np(&player->cond_queue, &player->mutex_queue, sleep_time/100011);
//        int timeout_ret = pthread_cond_timedwait(&player->cond_queue, &player->mutex_queue, sleep_time/100011);
...
int player_write_audio(struct DecoderData *decoder_data, JNIEnv *env, int64_t pts, uint8_t *data, int data_size, int original_data_size){
	...
	jbyteArray sample_byte_array = (*env)->NewByteArray(env, data_size);
	if(sample_byte_array == NULL){
    ...
    enum WaitFuncRet wait_ret = player_wait_for_frame(player, player->audio_clock+AUDIO_TIME_ADJUST_US, stream_no);
    if(wait_ret==WAIT_FUNC_RET_SKIP){
    	goto end;
    }
	LOGI(10, "player_write_audio Writing sample data");

	jbyte *jni_samples = (*env)->GetByteArrayElements(env, sample_byte_array, NULL);
	memcpy(jni_samples, data, data_size);
	(*env)->ReleaseByteArrayElements(env, sample_byte_array, jni_samples,0);
	LOGI(10, "player_write_audio playing audio track");
	ret = (*env)->CallIntMethod(env, player->audio_track, player->audio_track_write_method, sample_byte_array, 0, data_size);
	...
	(*env)->DeleteLocalRef(env, sample_byte_array);
	end:return err;
}
int player_decode_audio(struct DecoderData *decoder_data, JNIEnv *env, struct PacketData *packet_data){
	int got_frame_ptr;
	struct Player *player = decoder_data->player;
	int stream_no = decoder_data->stream_no;
	AVCodecContext * ctx = player->input_codec_ctxs[stream_no];
	...
	int original_data_size = av_samples_get_buffer_size(NULL, ctx->channels, frame->nb_samples, ctx->sample_fmt,1);
	uint8_t *audio_buf;
	int data_size;
	if(player->swr_context!=NULL){
		uint8_t *out[] = { player->audio_buf2 };
		int sample_per_buffer_divider = player->audio_track_channel_count * av_get_bytes_per_sample(player->audio_track_format);
		int len2 = swr_convert(player->swr_context, out, sizeof(player->audio_buf2) / sample_per_buffer_divider, 	(uint8_t const **)frame->data, frame->nb_samples);
		if (len2 < 0) {
	...
	int err;
	if((err=player_write_audio(decoder_data, env, pts, audio_buf, data_size, original_data_size))){
...
void player_decode_audio_flush(struct DecoderData * decoder_data, JNIEnv *env){
	struct Player *player = decoder_data->player;
	(*env)->CallVoidMethod(env, player->audio_track, player->audio_track_flush_method);
}
...
//    LOGI(2, "player_decode_video");
    ...
    ANativeWindow_setBuffersGeometry(window, ctx->width, ctx->height, WINDOW_FORMAT_RGBA_8888);
	...
	avpicture_fill((AVPicture *) rgb_frame, buffer.bits, out_format, buffer.width, buffer.height);
	...
	LOGI(7,	"Buffer: width: %d, height: %d, stride: %d",
	...
	if (ctx->pix_fmt == PIX_FMT_YUV420P) {
		...
		sws_context = sws_getCachedContext(sws_context, ctx->width, ctx->height, ctx->pix_fmt, ctx->width, ctx->height, out_format,	SWS_FAST_BILINEAR, NULL, NULL, NULL);
	...
	LOGI(10, "player_decode_video Decoded video frame: %f, time_base: %" SCNd64,
			time/1000000.0, pts);
...
void * player_decode(void *data){
    ...
    for(;;){
    	...
    	pthread_mutex_lock(&player->mutex_queue);
    	pop:packet_data = queue_pop_start_already_locked(&queue, &player->mutex_queue, &player->cond_queue, (QueueCheckFunc)player_decode_queue_check_func, decoder_data, (void **)&interrupt_ret);
        if(packet_data ==NULL){
        ...
        goto end_loop;
        stop:
		LOGI(3, "player_decode stop[%d]", stream_no);
		stop = TRUE;
		flush:
		LOGI(3, "player_decode flush[%d]", stream_no);
		...
		while((to_free = queue_pop_start_already_locked_non_block(queue))!=NULL){
            ...
            queue_pop_finish_already_locked(queue, &player->mutex_queue, &player->cond_queue);
		}
		LOGI(3, "player_decode flushing playback[%d]", stream_no);
		if(codec_type==AVMEDIA_TYPE_AUDIO){
		...
		LOGI(3, "player_decode flushed playback[%d]", stream_no);
		if(stop){
			LOGI(3, "player_decode stopping stream");
			player->stop_streams[stream_no] = FALSE;
			pthread_cond_broadcast(&player->cond_queue);
			pthread_mutex_unlock(&player->mutex_queue);
			goto detach_current_thread;
		}else{
			LOGI(3, "player_decode flush stream[%d]", stream_no);
    ...
    detach_current_thread:ret = (*player->get_javavm)->DetachCurrentThread(player->get_javavm);
    if(ret&&!err){
...
//	LOGI(2, "player_read_from_stream");
    ...
    if (ret) {
    	...
    	for (;;) {
    		...
    		if (ret < 0) {
    			pthread_mutex_lock(&player->mutex_queue);
    			LOGI(2, "player_read_from_stream stream end");
...
//    		LOGI(2, "player_read_from_stream Read frame");
    		pthread_mutex_lock(&player->mutex_queue);
    		if (player->stop) {
...
//    				LOGI(2, "player_read_from_stream stream found [%d]", stream_no);
    			}
    		}
    		...
    		*packet_data->packet = packet;
    		...
    		seek_loop:
    		LOGI(1, "seek_loop");
    		// setting stream that will be used as a base for seeking
    		...
    		seek_target = av_rescale_q(player->seek_position,  AV_TIME_BASE_Q, seek_input_stream->time_base);
    		LOGI(2, "player_read_from_stream seeking to: ""%fs, time_base: %" PRId64 "\n", player->seek_position / 1000000.0, seek_target);
    		...
    		LOGI(2, "player_read_from_stream seeking success");
...
int player_start_decoding_threads(struct Player *player){//重要
	pthread_attr_t attr;
	int ret ;
	int i;
	int err = 0;
    ...
    for(i=0;i<player->caputre_streams_no;++i){//caputre_streams_no = 2
    	struct DecoderData * decoder_data = malloc(sizeof(decoder_data));
    	*decoder_data = (struct DecoderData){
    		player:player,
    		stream_no:i
    	};
    	ret = pthread_create(&player->decode_threads[i], &attr, player_decode, decoder_data);
    	if(ret){
    ...
    ret = pthread_create(&player->thread_player_read_from_stream, &attr, player_read_from_stream, player);
    if(ret){
    	err = -ERROR_COULD_NOT_CREATE_PTHREAD;
    	goto end;
    }
...
//	LOGI(2, "player_stop_without_lock");
	int ret;
	struct Player *player = state->player;
	if(!player->playing){
		return;
...
int player_set_data_source(struct State *state, const char *file_path, AVDictionary *dictionary, int video_stream_no, int audio_stream_no, int subtitle_stream_no){
	LOGI(2, "player_set_data_source");
	struct Player *player = state->player;
	int err = ERROR_NO_ERROR;
	int i;
	pthread_mutex_lock(&player->mutex_operation);//锁定所指向的互斥锁
	...
	player->start_time = 0;
	player->pause_time = 0;
	if((err=player_create_context(player))<0){//AVFormatContext
		goto error;
	}
	if((err=player_create_interrupt_callback(player))<0){//指定AVIOInterruptCB
		goto error;
	}
	if((err=player_open_input(player, file_path, dictionary))<0){//avformat_open_input
		LOGI(2, "player_open_input error");
		goto error;
	}else{
		LOGI(2, "player_open_input ok");
	}
	if((err=player_find_stream_info(player))<0){//avformat_find_stream_info
		LOGI(2, "player_find_stream_info error");
		goto error;
	}else{
		LOGI(2, "player_find_stream_info ok");
	}
	player_print_video_informations(player, file_path);

	if((err=player_print_report_video_streams(state->env, player->thiz, player))<0){
		LOGI(2, "player_print_report_video_streams error");
		goto error;
	}else{
		LOGI(2, "player_print_report_video_streams ok");
	}
    if((player->video_stream_no=player_find_stream(player, AVMEDIA_TYPE_VIDEO, video_stream_no))<0){// 这里就可以或得视频宽高了 获取视频流的编号
    	LOGI(2, "player_find_stream video error");
    	err = player->video_stream_no;
    	goto error;
    }else{
		LOGI(2, "player_find_stream video ok");
    }

    if((player->audio_stream_no=player_find_stream(player, AVMEDIA_TYPE_AUDIO, audio_stream_no))<0){//获取音频流的编号
    	LOGI(2, "player_find_stream audio error");
    	err = player->audio_stream_no;
    	goto error;
    }else{
		LOGI(2, "player_find_stream audio ok");
    }
#ifdef SUBTITLES
...
#endif // SUBTITLES
	if((err=player_alloc_video_frames(state->env, player))<0){//AVFrame
		LOGI(2, "player_alloc_video_frames error");
		goto error;
	}else{
		LOGI(2, "player_alloc_video_frames ok");
	}
	if((err=player_alloc_frames(player))<0){
		LOGI(2, "player_alloc_frames error");
		goto error;
	}else{
		LOGI(2, "player_alloc_frames ok");
	}
    if((err=player_alloc_queues(state))<0){
    	LOGI(2, "player_alloc_queues error");
    	goto error;
    }else{
		LOGI(2, "player_alloc_queues ok");
    }
    struct DecoderState video_decoder_state = {
    		stream_no:player->video_stream_no,
    		player:player,
    		env:state->env
    };
#ifdef SUBTITLES
...
#endif // SUBTITLES
	if((err=player_create_audio_track(player, state))<0){//初始化AudioTrack 设置采样率 声道等信息
		LOGI(2, "player_create_audio_track error");
		goto error;
	}else{
		LOGI(2, "player_create_audio_track ok");
	}
    ...
    if((err = player_start_decoding_threads(player))<0){
    	LOGI(2, "player_start_decoding_threads error");
    	goto error;
    }else{
		LOGI(2, "player_start_decoding_threads ok");
    }
    //success
    player->playing = TRUE;
	LOGI(1, "player_set_data_source success");
	goto end;
	error:
	LOGI(1,"player_set_data_source error");
...
//	player_find_stream_info_free(player);
	...
	LOGI(0, "player_set_data_source end");
	pthread_mutex_unlock(&player->mutex_operation);
//	player_change_video_size(state->env, player);
	return err;
}
...
//setDataSourceNative(String url,Map<String, String> dictionary, int videoStreamNo,
//			int audioStreamNo, int subtitleStreamNo);
//dictionary ????
int jni_player_set_data_source(JNIEnv *env, jobject thiz, jstring string, jobject dictionary, int video_stream_no, int audio_stream_no,
		int subtitle_stream_no){
	LOGI(2, "jni_player_set_data_source");
	...
	struct Player * player = player_get_player_field(env, thiz);
	struct State state = {player:player, env:env};
	int ret = player_set_data_source(&state, file_path, dict, video_stream_no,audio_stream_no, subtitle_stream_no);
...
#warning "Profiler enabled"
...
#endif
	struct Player *player = malloc(sizeof(struct Player));//为player分配内存
	memset(player, 0, sizeof(player));//清空或初始化player
	player->audio_stream_no = -1;
	player->video_stream_no = -1;
#ifdef SUBTITLES
	player->subtitle_stream_no = -1;
	...
	jclass player_class = (*env)->FindClass(env, player_class_path_name);//FFmpegPlayer
	if (player_class == NULL) {
		err = ERROR_NOT_FOUND_PLAYER_CLASS;
		goto free_player;
	}
	jfieldID player_m_native_player_field = java_get_field(env, player_class_path_name, player_m_native_player);//int mNativePlayer
	...
	player->player_prepare_frame_method = java_get_method(env, player_class, player_prepare_frame);
	...
	player->player_on_update_time_method = java_get_method(env,	player_class, player_on_update_time);
	...
	player->player_set_stream_info_method = java_get_method(env, player_class, player_set_stream_info);
	if (player->player_set_stream_info_method == NULL) {
		err = ERROR_NOT_FOUND_SET_STREAM_INFO_METHOD;
		goto free_player;
	}
    player->player_set_video_size  = java_get_method(env, player_class, player_set_video_size);
    if(player->player_set_video_size==NULL){
    	err = ERROR_NOT_FOUND_SET_ON_VIDEO_SIZE;
    	goto free_player;
    }
	...
	if (player->audio_track_class == NULL) {
		...
		(*env)->DeleteLocalRef(env, audio_track_class);
		//int write(byte[] audioData, int offsetInShorts, int sizeInShorts);
		...
		player->audio_track_play_method = java_get_method(env,	player->audio_track_class, audio_track_play);
			...
			player->audio_track_get_channel_count_method = java_get_method(env,	player->audio_track_class, audio_track_get_channel_count);
			...
			pthread_mutex_init(&player->mutex_operation, NULL);//多线程互斥锁的初始化
			pthread_mutex_init(&player->mutex_interrupt, NULL);
			pthread_mutex_init(&player->mutex_queue, NULL);//互斥锁的初始化
#ifdef SUBTITLES
	pthread_mutex_init(&player->mutex_ass, NULL);
#endif
	pthread_cond_init(&player->cond_queue, NULL);//初始化一个条件变量
...
//	avformat_network_init();
	av_register_all();
//	register_jni_protocol(player->get_javavm);//???干什么用 目测跟网络有关，先注释掉
	player_print_all_codecs();
	goto end;

delete_audio_track_global_ref: (*env)->DeleteGlobalRef(env,	player->audio_track_class);
...
void jni_player_render_frame_start(JNIEnv *env, jobject thiz) {
	...
	LOGI(2, "jni_player_render_frame_start");

}
void jni_player_render_frame_stop(JNIEnv *env, jobject thiz) {
	LOGI(2, "jni_player_render_frame_stop");
	struct Player * player = player_get_player_field(env, thiz);

	struct State state = {player: player, env: env};

	LOGI(2, "jni_player_render_frame_stop stopping render");

	player_stop(&state);

	LOGI(2, "jni_player_render_frame_stop waiting for mutex");
	pthread_mutex_lock(&player->mutex_queue);
	if (player->window == NULL) {
		LOGE(1,	"jni_player_render_frame_stop Window is null this "
				"mean that you did not call render function");
		exit(1);
	}
	LOGI(2, "jni_player_render_frame_stop releasing window");
...
jlong jni_player_get_video_duration(JNIEnv *env, jobject thiz){
	LOGI(2, "jni_player_get_video_duration");
	struct Player *player = player_get_player_field(env, thiz);
	return player->video_duration;
}

void jni_player_render(JNIEnv *env, jobject thiz, jobject surface){
	LOGI(2, "jni_player_render");
	struct Player *player = player_get_player_field(env, thiz);
	ANativeWindow * window = ANativeWindow_fromSurface(env, surface);
	pthread_mutex_lock(&player->mutex_queue);
	if(player->window!=NULL){
		LOGE(1,	"jni_player_render Window have to be null before calling render function");
//		exit(1);
	}
...
jint Java_com_timeslily_ffmpeg_FFmpegPlayer_getDuration(JNIEnv *env, jobject this, jstring filePath){
	char * videoFileName;
